...
/**
 * https://github.com/paveldn/HaierProtocol/blob/main/include/protocol/haier_frame_types.h
 * 
 * @author liuyulong
 * @since 2025-04-24
 */
public enum HaierIoProtocolType {

	UNKNOWN_FRAME_TYPE(0x00), //
	CONTROL(0x01), // Requests or sets one or multiple parameters (module <-> device, required)
	STATUS(0x02), // Contains one or multiple parameters values, usually answer to control frame
					// (module <-> device, required)
	...
	CONFIRM(0x05), // Acknowledgment, usually used to confirm reception of frame if there is no
					// special answer (module <-> device, required)
	...
	GET_ALL_ADDRESSES_RESPONSE(0x68), // Answer to request of all devices addresses (module <- device , interactive,
										// optional)
	HANDSET_CHANGE_NOTIFICATION(0x69), // Handset change notification frame (module <- device , interactive,
										// optional)
	...
	GET_DEVICE_CONFIGURATION_RESPONSE(0x7D), // Response to device configuration request (module <- device,
	...
	UPLINK_TRANSPARENT_TRANSMISSION(0x8D), // Uplink transparent transmission (proxy data device -> Haier cloud)
											// (module <- device, interactive, optional)
	START_DEVICE_UPGRADE(0xE1), // Initiate device OTA upgrade (module -> device, OTA required)
	START_DEVICE_UPGRADE_RESPONSE(0xE2), // Response to initiate device upgrade command (module <- device, OTA
											// required)
	GET_FIRMWARE_CONTENT(0xE5), // Requests to send firmware (module <- device, OTA required)
	GET_FIRMWARE_CONTENT_RESPONSE(0xE6), // Response to send firmware request (module -> device, OTA required)
											// (multipacket?)
	...
	GET_UPGRADE_RESULT_RESPONSE(0xEF), // Response to firmware update results request (module -> device, OTA
										// required)
	GET_NETWORK_STATUS(0xF0), // Requests network status (module <- device, interactive, optional)
	GET_NETWORK_STATUS_RESPONSE(0xF1), // Response to network status request (module -> device, interactive,
										// optional)
	START_WIFI_CONFIGURATION(0xF2), // Starts WiFi configuration procedure (module <- device, interactive, required)
	START_WIFI_CONFIGURATION_RESPONSE(0xF3), // Response to start WiFi configuration request (module -> device,
												// interactive, required)
	STOP_WIFI_CONFIGURATION(0xF4), // Stop WiFi configuration procedure (module <- device, interactive, required)
	STOP_WIFI_CONFIGURATION_RESPONSE(0xF5), // Response to stop WiFi configuration request (module -> device,
											// interactive, required)
	REPORT_NETWORK_STATUS(0xF7), // Reports network status (module -> device, required)
	CLEAR_CONFIGURATION(0xF8), // Request to clear module configuration (module <- device, interactive,
								// optional)
	BIG_DATA_REPORT_CONFIGURATION(0xFA), // Configuration for auto report device full status (module -> device,
											// interactive, optional)
	BIG_DATA_REPORT_CONFIGURATION_RESPONSE(0xFB), // Response to set big data configuration (module <- device,
													// interactive, optional)
	GET_MANAGEMENT_INFORMATION(0xFC), // Request management information from device (module -> device, required)
	GET_MANAGEMENT_INFORMATION_RESPONSE(0xFD), // Response to management information request (module <- device,
												// required)
	 ...
	 * @param code
	 */
	private HaierIoProtocolType(int code) {
		this.code = code;
	}
