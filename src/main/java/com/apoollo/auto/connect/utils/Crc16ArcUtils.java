 ...
 * @since 2025-04-21
 */
public class Crc16ArcUtils {

	private static final short INITIAL_CRC = 0x00;

	// CRC-16/ARC lookup table
	private static final int[] CRC_TABLE = new int[] { 0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280, 0xC241,
	...
	public static short crc16(byte data, short crc) {
		int index = (crc ^ (data & 0xFF)) & 0xFF;
		return (short) ((crc >>> 8) ^ CRC_TABLE[index]);
	}

	// 字节数组CRC计算（指定初始值）
	public static short crc16(byte[] data, short initialVal) {
		short crc = initialVal;
		for (byte b : data) {
			crc = crc16(b, crc);
		}
		return crc;
